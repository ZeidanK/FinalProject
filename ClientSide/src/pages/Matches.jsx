...
import MatchesStatsRow from '../components/MatchesStatsRow'
    ...
    <>
      <Box sx={{ py: { xs: 3, md: 4 }, position: 'relative', overflow: 'hidden' }} role="region" aria-label="Matches page">
        ...
        <Container maxWidth={false} disableGutters sx={{ px: { xs: 2, sm: 3, md: 4, xl: 5 }, width: '100%', position: 'relative', zIndex: 1 }}>
          <Stack component={motion.div} variants={containerVariants} initial="hidden" animate="show" spacing={2.5}>
            <PageHeaderCard title="Matches" description="Match invoices to bank transactions for reconciliation. Select one invoice and one transaction, then create a match." onRefresh={loadData} refreshDisabled={loading} variants={itemVariants} />

            <MatchesStatsRow unmatchedInvoices={invoices.length} unmatchedTransactions={table.filteredTransactions.length} inReviewCount={inReviewCount} totalMatches={matches.length} totalMatchedFormatted={fmtAmount(table.totalMatchedAmount)} />
